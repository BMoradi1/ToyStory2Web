...
(async () => {
  ...
  // Capture a gamepad button through the host's real polling path.
  const nativePads=navigator.getGamepads.bind(navigator);
  let pressed=-1;
  Object.defineProperty(navigator,'getGamepads',{configurable:true,value:()=>[{connected:true,axes:[0,0],buttons:Array.from({length:16},(_,i)=>({pressed:i===pressed}))}]});
  ts2.frontDrive(0x4000);ts2.frontDrive(0);
  ts2.frontDrive(0x4000);ts2.frontDrive(0,30);
  pressed=1;ts2.frontDrive(0);pressed=-1;ts2.frontDrive(0);
  if(ts2.front.state.value.pad.up[0]!==1||ts2.front.state.value.pad.fire[0]!==12)throw Error('pad capture/swap failed');
  window.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',code:'Enter'}));
  window.dispatchEvent(new KeyboardEvent('keyup',{key:'Enter',code:'Enter'}));
  if(JSON.parse(localStorage.getItem('ts2.pad')).up[0]!==1)throw Error('pad bindings did not persist');
  Object.defineProperty(navigator,'getGamepads',{configurable:true,value:nativePads});
  ...
  ts2.frontDrive(0,30);ts2.frontDrive(0x4000);
  if(ts2.save.lives===7)throw Error('load applied before its fade');
  ts2.frontDrive(0,43);
  ...
  Storage.prototype.setItem=function(k,v){if(k==='ts2.slot.0')throw new DOMException('quota','QuotaExceededError');return nativeSet.call(this,k,v);};
  ts2.frontDrive(0,30);ts2.frontDrive(0x4000);ts2.frontDrive(0,43);
  await wait(()=>ts2.front.state?.message==='could not save game','failed save was not reported');
  Storage.prototype.setItem=nativeSet;
  if(ts2.front.state.page!=='save'||localStorage.getItem('ts2.slot.0'))throw Error('failed save did not stay retryable');
  ts2.frontDrive(0,40);ts2.frontDrive(0x4000);
  if(localStorage.getItem('ts2.slot.0'))throw Error('save committed before fade');
  ts2.frontDrive(0,43);
  await wait(()=>ts2.front.screen==='menu','save did not return to main menu');
  ...
  ts2.frontDrive(0x4000);ts2.frontDrive(0,43);
  await wait(()=>ts2.front.screen==='menu','load did not return');
  await choose(1);
  ts2.frontDrive(0x4000);ts2.frontDrive(0);
  for(let i=0;i<6;i++){window.dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowDown',code:'ArrowDown'}));ts2.frontDrive(0);window.dispatchEvent(new KeyboardEvent('keyup',{key:'ArrowDown',code:'ArrowDown'}));}
  if(ts2.front.state.subrow!==6||ts2.front.state.controlTop!==2)throw Error('physical arrow navigates twice or fails to scroll');
